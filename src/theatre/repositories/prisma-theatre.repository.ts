...
export class PrismaTheatreRepository {
    ...
    async create(createTheatreDto: CreateTheatreDto): Promise<TheatreEntity> {
        try {
            ...
            if (auditoriums.length > 0) {
                const auditoriumPromises = auditoriums.map((auditorium) => this.createAuditorium(createdTheatre, auditorium))
    ...
    async findAllWithEverything(): Promise<TheatreEntity[]> {
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                throw new Error(`Failed to fetch theatres with details: ${error.message}`);
            }
            throw error;
        }
    }
    ...
    async findOneWithEverything(id: string): Promise<TheatreEntity> {
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                ...
                throw new Error(`Failed to fetch theatre with details: ${error.message}`);
            }
            throw error;
        }
    }

    async update(id: string, updateTheatreDto: CreateTheatreDto): Promise<TheatreEntity> {
        try {
    ...
    private async createAuditorium(createdTheatre: TheatreEntity, auditorium: CreateAuditoriumInput) {
        try {
            ...
            const seatsData = ALL_ROWS.flatMap((row) => Array.from({ length: 18 }).map((_, index) => ({
            ...
            if (count) {
                console.log(`Created ${count} seats for auditorium ${createdAuditorium.number}`);
            }
        } catch (error) {
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
